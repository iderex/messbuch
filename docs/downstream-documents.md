...
| The ruleset on the default branch | `docs/required-checks.md`, whose whole premise is what that ruleset currently requires, `docs/release-checklist.md`, one of whose items is whether that ruleset requires anything at all, and `docs/supply-chain.md`, whose branch-protection and code-review entries are triage of what the audit says about that ruleset |
| An action pin, the dependency set, or what the gate does with `GOFLAGS` | `docs/supply-chain.md`, which counts the pins and states what the locked restore covers, so a tag creeping back into a `uses:` line leaves a count there that no longer holds |
...
| The coverage floor's number, its reason, or the surface it is measured over | `coverage-floor.toml` itself, which is where all three are written and is the authority rather than a copy, and `docs/quality-parity.md`, whose coverage-bar row says where the bar is and what it does not reach |
| The legs of the single local gate command, or the command itself | `docs/quality-parity.md`, whose section on which legs are in place is a statement about what that command runs and about what a pull request runs, and `CONTRIBUTING.md`, which names that command and nothing else as the local gate |
| The license of the code or of the corpus, whether contributions from outside are accepted, or how a commit is signed off | `CONTRIBUTING.md`, which states all four and is the document a contributor reads before doing the work rather than after |
| The private reporting channel, or where the line falls between a vulnerability and a data defect | `SECURITY.md`, which names the channel and puts a wrong number in the corpus on the public tracker on purpose. `.github/ISSUE_TEMPLATE/wrong-number.yml` and `docs/corrections.md` are the other side of that line |
...
| Which reference value a deviation is measured against, the ratio at which a measurement is admitted to the deviation distribution, or what the statistic does with a measurement whose uncertainty was not quoted | `docs/decisions/0014-deviation-statistic.md`, which fixes all three and requires the reference, its vintage and the excluded counts in every output, and `docs/decisions/0011-corpus-versioning.md`, whose stamp is where they are printed |
| Which event counts as a tension, how an unresolved one is counted, or what an output format must carry beside a probability | `docs/decisions/0015-what-counts-as-a-tension.md`, which fixes the three events, the four outcome states and the members no output may drop, and `docs/decisions/0014-deviation-statistic.md`, whose reference and admission rule are two of those members |
...
| Adding, superseding or renumbering any decision record | `docs/decisions/README.md`. An index that does not list a record is wrong in the one place a reader goes to find records. Also `docs/release-checklist.md`, whose item 3 pastes a count of that directory, so every record added leaves a number there that was true on the day it was written |
...
The build, the toolchain pin and the single local gate command have a row in the
map above, and both halves of it now point at documents that exist. `#20` landed
`CONTRIBUTING.md`, which names the command and nothing else as the local gate,
and `SECURITY.md`. The third document that issue asks for, a code of conduct, is
