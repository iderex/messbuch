...
| `Build and test` | `.github/workflows/build-and-test.yml` | Whatever any leg of `go run . ci` refuses, because it invokes the whole command rather than one leg. This row does not list those legs, because a list here goes stale the day one is added and the two most recent additions found it stale. The run prints its own covered set and the leg it stopped at. | Yes. Triggered on `pull_request` with the default types and no branch, path or base filter. |
